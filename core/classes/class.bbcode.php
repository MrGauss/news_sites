...
class bbcode
{
    ...
    final public static function bbcode2html( $text = false )
    {
        ...
        $text = preg_replace_callback( '!\[(youtube)(\|.+?|)\](.+?)\[\/\1\]!is', 'self::process_youtube', $text );
        return $text;
    }
    ...
    final public static function html2bbcode( $text = false )
    {
        ...
        $text = preg_replace_callback( '!<div class=\"bb_youtube\">(.+?)src=\"(\S+?)\"(.+?)<\/div>!is', 'self::process_youtube', $text );
        return $text;
    }

    final private static function process_youtube( $array )
    {
        if( strpos( isset($array[0])?$array[0]:'', '[/youtube]' ) !== false )
        {
            $video = array();
            $video['src']     = isset($array[3])?$array[3]:false;

            return '<div class="bb_youtube"><iframe width="640" src="'.$video['src'].'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
        }

        if( strpos( isset($array[0])?$array[0]:'', '<div class="bb_youtube"' ) !== false )
        {
            $video = array();
            $video['src']     = preg_replace( '!(.*)src=\"(\S+?)\"(.*)!is', '$2', $array['0'] );
            $video['src']     = isset($video['src'][2])?$video['src'][2]:false;

            return false;

            return '[youtube]'.$video['src'].'[/youtube]';
        }
    }
